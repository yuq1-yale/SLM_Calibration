...
if num_boards_found.value == 1:
    ...
    if width == 1024:
        load_lut_status = slm_lib.Load_LUT_file(board_number, b"C:\\Program Files\\Meadowlark Optics\\Blink Plus\\Cal Kit\\251106_calibrated_SLM.LUT")
    if load_lut_status == 0:
        ...
        if(retVal != 1):
            ...
            Variable = 255
            StepBy = -1
            PixelsPerStripe = 4 # Use a fairly high frequency pattern to separate the 0th from the 1st order
            bVertical = 1
            ...
            for region in range(0, NumRegions):
                ...
                for DataPoint in range(0, NumDataPoints):
                    ...
                    power = pm.capture(10, 'uW')
                    DI_Intensities[DataPoint] = power
                    sleep(0.2)

                print("\n") 
                FileName = f"./measured_data/Verify_{region}_251106_calibrated_SLM.csv"
